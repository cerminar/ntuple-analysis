...
class BaseHistos():
    def __init__(cls, name, root_file=None):
        if root_file is not None:
            for histo in root_file.GetListOfKeys():
                if name in histo.GetName():
                    # print histo.GetName(), histo.GetName().split('_')[2]
                    setattr(cls, 'h_'+histo.GetName().split('_')[2], root_file.Get(histo.GetName()))
        else:
...
class DigiHistos(BaseHistos):
    def __init__(cls, name, root_file=None):
        if not root_file:
            cls.h_layer = ROOT.TH1F(name+'_layer', 'Digi layer #', 60, 0, 60)
            cls.h_simenergy = ROOT.TH1F(name+'_energy', 'Digi sim-energy (GeV)', 100, 0, 2)
        BaseHistos.__init__(cls, name, root_file)

    def fill(cls, digis):
        rnp.fill_hist(cls.h_layer, digis.layer)
        rnp.fill_hist(cls.h_simenergy, digis.simenergy)


class TCHistos(BaseHistos):
    def __init__(cls, name, root_file=None):
        if not root_file:
            cls.h_energy = ROOT.TH1F(name+'_energy', 'TC energy (GeV)', 100, 0, 2)
            ...
            cls.h_layerVenergy = ROOT.TH2F(name+'_layerVenergy', "Energy (GeV) vs Layer #", 60, 0, 60, 100, 0, 2)
            cls.h_energyVeta = ROOT.TH2F(name+'_energyVeta', "Energy (GeV) vs Eta", 100, -3.5, 3.5, 100, 0, 2)
            cls.h_energyVetaL1t5 = ROOT.TH2F(name+'_energyVetaL1t5', "Energy (GeV) vs Eta (layers 1 to 5)", 100, -3.5, 3.5, 100, 0, 2)
            cls.h_energyVetaL6t10 = ROOT.TH2F(name+'_energyVetaL6t10', "Energy (GeV) vs Eta (layers 6 to 10)", 100, -3.5, 3.5, 100, 0, 2)
            cls.h_energyVetaL11t20 = ROOT.TH2F(name+'_energyVetaL11t20', "Energy (GeV) vs Eta (layers 11 to 20)", 100, -3.5, 3.5, 100, 0, 2)
            cls.h_energyVetaL21t60 = ROOT.TH2F(name+'_energyVetaL21t60', "Energy (GeV) vs Eta (layers 21 to 60)", 100, -3.5, 3.5, 100, 0, 2)
            cls.h_energyPetaVphi = ROOT.TProfile2D(name+'_energyPetaVphi', "Energy profile (GeV) vs Eta and Phi", 100, -3.5, 3.5, 100, -3.2, 3.2)
    ...
    def fill(cls, tcs):
        ...
        rnp.fill_hist(cls.h_layerVenergy, tcs[['layer', 'energy']])
        rnp.fill_hist(cls.h_energyVeta, tcs[['eta', 'energy']])
        rnp.fill_hist(cls.h_energyVeta, tcs[['eta', 'energy']])
        rnp.fill_hist(cls.h_energyVetaL1t5, tcs[(tcs.layer >= 1) & (tcs.layer <= 5)][['eta', 'energy']])
        rnp.fill_hist(cls.h_energyVetaL6t10, tcs[(tcs.layer >= 6) & (tcs.layer <= 10)][['eta', 'energy']])
        rnp.fill_hist(cls.h_energyVetaL11t20, tcs[(tcs.layer >= 11) & (tcs.layer <= 20)][['eta', 'energy']])
        rnp.fill_hist(cls.h_energyVetaL21t60, tcs[(tcs.layer >= 21) & (tcs.layer <= 60)][['eta', 'energy']])
        rnp.fill_profile(cls.h_energyPetaVphi, tcs[['eta', 'phi', 'energy']])
...
class Cluster3DHistos(BaseHistos):
    def __init__(cls, name, root_file=None):
        if not root_file:
            ...
            cls.h_showlenght = ROOT.TH1F(name+'_showlenght', '3D Cluster showerlenght', 60, 0, 60)
