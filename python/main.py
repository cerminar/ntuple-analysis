...
def main(analyze, submit_mode=False):
    ...
    parser.add_option(
        "-n", "--nevents", dest="NEVENTS", help="# of events to process per sample (default is 10)", default=10
    )
    parser.add_option(
        "-b", "--batch", action="store_true", dest="BATCH", default=False, help="submit the jobs via CONDOR"
    )
    parser.add_option(
        "-r", "--run", dest="RUN", default=None, help="the batch_id to run (need to be used with the option -b)"
    )
    parser.add_option("-o", "--outdir", dest="OUTDIR", default=None, help="override the output directory for the files")
    if submit_mode:
        parser.add_option(
            "-l", "--local", action="store_true", dest="LOCAL", default=False, help="run the batch on local resources"
        )
        parser.add_option("-j", "--jobworkers", dest="WORKERS", default=2, help="# of local workers")
        parser.add_option("-w", "--workdir", dest="WORKDIR", help="local work directory")
