...
class Event(object):
    ...
    #     # for idx in range(0, len(branches)):
    #     #     # print '   idx: {} names[idx] {} branches[idx] {}, nd_array[branches[idx] {}'.format(idx, names[idx], branches[idx], nd_array[branches[idx]])
    #         # df[names[idx]] = nd_array[branches[idx]][0]
    #     print df[:10]
    #     print branches
    #     if '{}_n'.format(prefix) in branches:
    #         if df.shape[0] != nd_array['{}_n'.format(prefix)][0]:
    #             print 'ERROR in conversion: DF size: {} while n of objects in tree: {}'.format(df.shape[0], nd_array['{}_n'.format(prefix)][0])
    #         else:
